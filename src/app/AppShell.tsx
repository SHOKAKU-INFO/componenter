...
const nav: { to: string; label: string; icon: IconName }[] = [
  { to: '/app', label: 'ホーム', icon: 'home' }, { to: '/components', label: 'コンポーネント', icon: 'grid' },
...
export default function AppShell() {
  ...
  const title = location.pathname === '/create' ? 'コンポーネント作成' : nav.find(n => location.pathname===n.to||location.pathname.startsWith(`${n.to}/`))?.label ?? '設定';
  return <div className="app-shell">
    <aside className={`sidebar ${open ? 'is-open' : ''}`}>
      <NavLink to="/" className="brand"><span className="brand-mark"><Icon name="layers" size={19}/></span><span>Componenter</span></NavLink>
      ...
      <div className="sidebar-bottom">
        <NavLink to="/settings" onClick={() => setOpen(false)}><Icon name="settings"/><span>アカウント設定</span></NavLink>
        <div className="sync-card"><span className={`status-dot ${user?'online':''}`}/><div><strong>{user?'クラウド同期中':'ローカル保存中'}</strong><small>{user?user.email:'ログインすると作品を同期'}</small></div></div>
      </div>
    </aside>
